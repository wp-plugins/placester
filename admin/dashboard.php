...
<div style="width: 970px" class="wrap"> 
  ...
  <?php placester_postbox_container_header('width:320px'); ?>
  	...
  	<?php placester_postbox('about-us', 'About Us', 'this is a test') ?>
  <?php placester_postbox_container_footer(); ?>
...
<?php if (get_option('placester_company_id') && get_option('placester_api_key')): ?>
    <script>
        jQuery(document).ready(function($) {
           line_widget('linechart', "<?php echo sha1("line_leads" . get_option('placester_company_id') . get_option('placester_api_key')); ?>", "<?php echo get_option('placester_company_id') ?>"); 
           pie_widget('piechart', "<?php echo sha1("pie_publishers" . get_option('placester_company_id') . get_option('placester_api_key')); ?>", "<?php echo get_option('placester_company_id') ?>"); 
