...
com.redfin.FastMarkerOverlay.prototype.draw = function() {
  ...
  while (i--) {
    ...
    textArray.push(marker._htmlTextArray);
    textArray.push("</div>");
  }
  ...
  while (i--) {
    var marker = this._markers[i];
    this._setMarkerHandlers(marker);
  }
}

com.redfin.FastMarkerOverlay.prototype._setMarkerHandlers = function(marker) {
  var markerData = marker.customData;
  var clickFunctor = this.markerClickFunctor;
  var hoverFunctor = this.markerHoverFunctor;

  // no jQuery here - otherwise memory leaks will occur
  document.getElementById(marker._id).onclick = 
      function()
      {
        clickFunctor(markerData);
      };
  document.getElementById(marker._id).onhover = 
      function()
      {
        hoverFunctor(markerData);
      };
}
