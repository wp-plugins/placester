...
class Tax_Meta_Class {
	...
	public function __construct ( $meta_box ) {
		...
		$this->SelfPath = trailingslashit(PL_THIRD_PARTY_URL) . 'tax-meta-class';
	...
	public function load_scripts_styles() {
		...
		*/
		...
		if (in_array($taxnow,$this->_meta_box['pages'])) {
			...
			wp_enqueue_style( 'tax-meta-clss', $this->SelfPath . '/css/tax-meta-class.css' );
			// Enqueue Meta Box Scripts
			wp_enqueue_script( 'tax-meta-clss', $this->SelfPath . '/js/tax-meta-clss.js', array( 'jquery' ), null, true );
	...
	public function add() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			...
			add_action($page.'_edit_form_fields',array( $this, 'show_edit_form_fields' ));
			//edit form
			add_action($page.'_edit_form',array( $this, 'show_form' ));
			//add fields to add new form
			add_action($page.'_add_form_fields',array( $this, 'show_new_form_fields' ));
			//new form
			add_action($page.'_add_form',array( $this, 'show_form' ));
	 ...
	 * @access public
	 */
	public function show_new_form_fields($term_id) {
		$this->_form_type = 'new';
	 ...
	 * @access public
	 */
	public function show_edit_form_fields($term_id) {
	...
	public function show_field_repeater( $field, $meta ) {
		...
		if (count($meta) > 0 && is_array($meta) ) {
			foreach ($meta as $me) {
				...
				echo '" alt="'.__('Remove','tax-meta').'" title="'.__('Remove','tax-meta').'" id="remove-'.$field['id'].'"></span>';
				$c = $c + 1;
	...
	public function show_field_textarea( $field, $meta ) {
		$this->show_field_begin( $field, $meta );
		echo "<textarea class='at-textarea large-text' style='{$field['style']}' name='{$field['id']}' id='{$field['id']}' rows='10'>{$meta}</textarea>";
		$this->show_field_end( $field, $meta );
	}
	...
	public function show_field_image( $field, $meta ) {
		...
		if (is_array($meta) && isset($meta['src']) && $meta['src'] != '') {
			$html .= "<span class='mupload_img_holder'><img src='".$meta['src']."' alt='uploaded image' /></span>";
			...
			$html .= "<input class='button at-upload_image_button' type='button' rel='".$field['id']."' value='Upload Image' />";
		}
		echo $html;
		$this->show_field_end( $field, $meta );
	}
