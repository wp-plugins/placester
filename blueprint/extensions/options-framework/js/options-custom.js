...
jQuery(document).ready(function($) {
	...
	if ($(activetab).filter('.group').length)
		$(activetab).filter('.group').fadeIn();
	else
		$('.group:first').fadeIn();
	...
	$('.fls-add-listing').live('click', function (event) {
		...
		if ($(this).hasClass('for_slideshow')) {
			var html = "<li style='float:left; list-style-type: none;'><div id='pls-featured-text' style='width: 200px; float: left;'>" + text
				+ "</div><a style='float:left;' href='#' id='pls-option-remove-listing'>Remove</a><input type='hidden' name='"
				+ option_name + "[" + iterator + "][" + key + "]=' value='" + text + "' /></li>";
		} else {
			var html = "<li style='float:left; list-style-type: none;'><div id='pls-featured-text' style='width: 200px; float: left;'>" + text
				+ "</div><a style='float:left;' href='#' id='pls-option-remove-listing'>Remove</a><input type='hidden' name='"
				+ option_name + "[" + key + "]=' value='" + text + "' /></li>";
		}
