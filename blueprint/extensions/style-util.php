	...
	private static function make_style($style, $value, $important = false)
	{
		...
		switch ($style) {
			...
			case 'bg_gradient':
				...
				$item = "background: $specified_color$important;\n"; // Old browsers
				$item .= "filter: progid:DXImageTransform.Microsoft.gradient(startColorstr='$specified_color', endColorstr='$tinted_color')$important;\n"; // IE6-9
				$item .= "background: -webkit-gradient(linear, left top, left bottom, from($specified_color), to($tinted_color))$important;\n"; // Chrome, Safari4+
				$item .= "background: -moz-linear-gradient(top, $specified_color, $tinted_color)$important;\n"; // FF3.6+
				$item .= "background: -ms-linear-gradient(top, $specified_color, $tinted_color)$important;\n"; // IE10+
				$item .= "background: -o-linear-gradient(top, $specified_color, $tinted_color)$important;\n"; // Opera11.10+
				$item .= "background: -webkit-linear-gradient(top, $specified_color, $tinted_color)$important;\n"; // Chrome10+, Safari5.1+
				$item .= "background: linear-gradient(top, $specified_color, $tinted_color)$important;\n"; // W3C
				return $item;
				break;

			case 'radius':
				$item = "-moz-border-radius: " . $value . "px" . $important . ";\n";
				$item .= "-webkit-border-radius: " . $value . "px" . $important . ";\n";
				$item .= "border-radius: " . $value . "px" . $important . ";\n";
				return $item;
				break;

			case 'background-image':
				return "background-image: url('" . $value . "')" . $important . ";\n";
				break;

			case 'background':
				return $style . ": " . $value . $important . ";\n" . "filter: none !important;\n";
