...
class PLS_Slideshow {
    ...
    static function slideshow( $args = '' ) {
        ...
        //having no data means we're have a gen 2 slideshow
        if ( !$data || ! is_array($data) ) {
            ...
            if ( pls_has_plugin_error() && current_user_can( 'administrator' ) ) {
                ...
                if ($allow_user_slides && $user_slides_header_id) {
                    ...
                    $data = self::handle_mixed_slideshow( $args );

                } else {
                    
                    $data = self::handle_listings_slideshow($args );
                }
            }
        }
    ...
    function handle_mixed_slideshow ( $args ) {
        ...
        //generate slides out of random listings. 
        if ( empty($data['images']) ) {
           $data = self::handle_empty();
        }

        return $data;
    }

    function handle_listings_slideshow () {
       if ($featured_option_id) {
        ...
        foreach ($listings as $index => $listing) {
            self::create_listing_slide( $listing, &$data );
        }
    ...
    function create_listing_slide ( $listing, &$data) {
